...
export class BinBrowser {
    ...
    async launch(options: LaunchOptions = {}): Promise<void> {
        ...
        if (DEV_MODE) { console.log(`Launching browser: ${executablePath}`); console.log(`Args: ${args.join(' ')}`); }
        ...
        if (this.process.stderr) {
            this.process.stderr.on('data', (data) => {
                if (DEV_MODE) { console.warn(`Browser stderr: ${data}`); }
            });
        }
    ...
    private async waitForDebugPort(): Promise<void> {
        ...
        if (DEV_MODE) { console.log('Waiting for browser debug port...'); }

        for (let i = 0; i < maxAttempts; i++) {
            try {
                const response = await fetch(`http://localhost:${this.debugPort}/json/version`);
                if (response.ok) {
                    if (DEV_MODE) { console.log('Browser debug port is ready'); }
                    return;
                }
            } catch (error) {
                if (i % 10 === 0) {
                    if (DEV_MODE) { console.log(`Still waiting for debug port... (attempt ${i + 1}/${maxAttempts})`); }
                }
            }
