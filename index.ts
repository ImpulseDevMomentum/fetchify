...
import { DEV_MODE } from './src/util/constants';
...
program
    ...
    .action(async (url: string, options: { output?: string; verbose?: boolean; amount?: string; download?: boolean }) => {
        try {
            ...
            if (DEV_MODE) { console.log('Initializing browser...'); }
...
program.command('track')
    .description('Fetch a Spotify track or YouTube video')
    .argument('<url>', 'Spotify track URL or YouTube video URL')
    .option('-o, --output <path>', 'Output directory/file (JSON format)')
    .option('-v, --verbose', 'Enable verbose logging')
    ...
    .action(async (url: string, options: { 
        ...
        try {
            ...
            const isSpotifyUrl = url.includes('open.spotify.com/track/');
            const isYouTubeUrl = url.includes('youtube.com') || url.includes('youtu.be');
            
            if (!isSpotifyUrl && !isYouTubeUrl) {
                console.error('Error: Please provide a valid Spotify track URL or YouTube video URL');
                console.error('Supported formats:');
                console.error('  Spotify: https://open.spotify.com/track/...');
                console.error('  YouTube: https://www.youtube.com/watch?v=...');
                console.error('  YouTube: https://youtu.be/...');
                process.exit(1);
            }
...
program
    ...
    .action(() => {
        console.log(ansci_logo);
        console.log('Fetchify - Spotify & YouTube Fetcher');
        console.log('Fetch Spotify playlist/track data and YouTube video data without using official APIs');
        console.log('\nFeatures:');
        console.log('- Fetch track information from Spotify playlists and tracks');
        console.log('- Fetch video information from YouTube videos');
        console.log('- Download MP3 files from YouTube');
        console.log('- Export to JSON format');
        console.log('- Configurable track limits');
        console.log('- Fetch track covers and video thumbnails');
        console.log('- Fetch extended metadata (release dates, albums, etc.)');
